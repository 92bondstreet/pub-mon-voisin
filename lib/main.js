		  ...
		  /**
			...
			*/
			...
			{	
    			...
    			/*if(args.length===0)
    				throw new Error('No valid args for getCatalogs');    				
    			else if(args.length===1)
    			{
    				callback = arguments[0];
    				if(Object.prototype.toString.call(callback) === "[object Function]")
    					return callback(new Error('JSON object to define WHO (recipient) is missing'));
    				else
    					this.checkRecipient(who);
    			...
    			else if(args.length===2)
    			{
    				...
    				throw new Error('Too many args for getCatalogs'); */   				
    			...
    			// 3. Parse catalogs
    			var self = this;
    			var file = __dirname + '/catalogs.json';
				fs.readFile(file, 'utf8', function (err, data) {
  					if (err) {
  						return callback(new Error(err));    					
  					}

  					self.freecatalogs = JSON.parse(data);  	
  					self.parseCatalogs();
			...
			*/
			checkRecipient:function (who){
				if(who !== undefined){
					if(	who.hasOwnProperty('title') &&
						who.hasOwnProperty('firstname') &&
			...
			*/
			pubLeVoisin: function(catalog)
			{
				...
				needle.post(url, dataPost, function(error, response, body){  
					fs.writeFile("body.html", body, function(err) {
					    if(err) {
					        console.log(err);
					    } else {
